...
int Matriz::NotVisited()
{
  ifstream arq, matt;
  char e;
  int a=0, size=Tam(), i, j;

  matt.open("dataset/input.data");
  matt >> a >> a >> nummat;
  arq.open("dataset/output.data");
  arq >> a >> a >> nummat;
...
int Matriz::WalkMatrix(int **mtrx, int im, int jm, int rsize, int n)
{
  ...
  while (yy==1 && vida!=0)
  {
    ...
    if (mtrx[im-1][jm+1]==-2 && mtrx[im][jm+1]==-2 && mtrx[im+1][jm+1]==-2 && mtrx[im+1][jm]==-2 && mtrx[im+1][jm-1]==-2 && mtrx[im][jm-1]==-2 && mtrx[im-1][jm-1]==-2 && mtrx[im-1][jm]==-2)
    {
      abc=3;
      break;
    }
